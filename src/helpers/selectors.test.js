...
test("1- getInterviewersForDay returns an array", () => {
  const result = getInterviewersForDay(state, "Monday");
  expect(Array.isArray(result)).toBe(true);
});

test("2- getInterviewersForDay returns an array with a length matching the number of Interviewers for that day", () => {
  const result = getInterviewersForDay(state, "Monday");
  expect(result.length).toEqual(1);
});

test("3- getInterviewersForDay returns an array containing the correct Interviewer objects", () => {
  const [first, second] = getInterviewersForDay(state, "Tuesday");
  expect(first).toEqual(null);
  expect(second).toEqual(2);
});

test("4- getInterviewersForDay returns an empty array when the days data is empty", () => {
  const result = getInterviewersForDay({ days: [] }, "Monday");
  expect(result.length).toEqual(0);
});

test("5- getInterviewersForDay returns an empty array when the day is not found", () => {
